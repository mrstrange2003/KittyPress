...
void decompressFile(const string &inputPath, const string &outputPath) {
    ...
    // 2️⃣ Read encoded bitstream length
    size_t encodedLen;
    in.read(reinterpret_cast<char*>(&encodedLen), sizeof(encodedLen));

    // 3️⃣ Read bits via BitReader
    BitReader reader(in);
    bool bit;
    string bitstream;
